...
#[derive(Debug, Clone)]
pub struct DynamicUnsized<Rw: RawRwLock + 'static, const SIZE: usize, const MUTABLE: bool = false> {
    content: Arc<RwLock<Rw, Arc<[u8]>>>
}
impl<Rw: RawRwLock + 'static, const SIZE: usize, const MUTABLE: bool> File for DynamicUnsized<Rw, SIZE, MUTABLE> {
    fn open(&self, _read: bool, write: bool, append: bool, truncate: bool) -> Result<Arc<dyn FileDescriptor>, FloreumError> {
        if truncate | append | (write & !MUTABLE) {
            ...
            Ok(Arc::new(DynamicUnsizedMutableDescriptor::<Rw, SIZE, MUTABLE> {
                file: self.content.clone(),
                content: RwLock::new(**self.content.read()),
            }))
        } else {
            Ok(Arc::new(DynamicUnsizedDescriptor::<SIZE>(self.content.read().clone())))
        }
    }
...
pub struct DynamicUnsizedDescriptor<const SIZE: usize>(Arc<[u8; SIZE]>);
impl<const SIZE: usize> FileDescriptor for DynamicUnsizedDescriptor<SIZE> {
...
pub struct DynamicUnsizedMutableDescriptor<Rw: RawRwLock + 'static, const SIZE: usize, const MUTABLE: bool = false> {
    file: Arc<RwLock<Rw, Arc<[u8; SIZE]>>>,
    content: RwLock<Rw, [u8; SIZE]>,
}
impl<Rw: RawRwLock + 'static, const SIZE: usize, const MUTABLE: bool> FileDescriptor for DynamicUnsizedMutableDescriptor<Rw, SIZE, MUTABLE> {
    fn read(&self, offset: u64, _count: u64) -> Result<(Arc<[u8]>, usize), FloreumError> {
